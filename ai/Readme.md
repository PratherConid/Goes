...
- `POST /move` — runs MCTS from the given position and returns the chosen move.
  - **Request fields:**
    - `config`: object with the game configuration (matches `shared/types.ts`'s `GameConfig.toJSON()` wire shape):
      - `boardType`: `"rect"` | `"rectd"` | `"cublat"` | `"hcub"` | `"tri"` | `"regpoly"` | `"tetra"` | `"dodeca"` | `"icosa"` | `"trihex"` | `"hex"` | `"hexdel"` | `"snubsq"` | `"snubsqtri"` | `"twsq"` | `"gtsq"` | `"star"`
        - `"regpoly"` (`regularPolygonBoard(n)`, a regular n-gon), `"tetra"` (`tetrahedronBoard()`, a regular tetrahedron - 4 vertices, all mutually adjacent), `"dodeca"` (`dodecahedronBoard()`), `"icosa"` (`icosahedronBoard()`), and `"star"` (`starBoard(n)`, 1 center node connected to n outer nodes) all have inherently-irrational unit-edge-length coordinates, which `BoardConfig::embed` (`unsigned` integers only) can't represent exactly like every other board type here can - so all five use `emb_dim = 0` with an empty `embed[]` per node instead: adjacency alone is exact and complete for gameplay, and no featurizer needs real coordinates for them (CNN/UNet require `emb_dim == 2` and throw otherwise - see `cnn.cpp`/`unet.cpp` - and none is grid-shaped anyway, so nothing is lost by omitting positions). A side-length-w subdivision of `tetra`'s 4 triangular faces is available via the `triangleForm`/`triform` board modifier (`{kind: "TriangleForm", w: int}`) instead of being built into `tetra` itself.
