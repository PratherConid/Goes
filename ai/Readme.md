...
- `POST /move` — runs MCTS from the given position and returns the chosen move.
  - **Request fields:**
    - `config`: object with the game configuration (matches `shared/types.ts`'s `GameConfig.toJSON()` wire shape):
      - `boardType`: `"rect"` | `"rectd"` | `"cub"` | `"hcub"` | `"tri"` | `"regpoly"` | `"dodeca"` | `"icosa"` | `"trihex"` | `"hex"` | `"hexdel"` | `"snubsq"` | `"snubsqtri"` | `"twsq"` | `"gtsq"`
        - `"regpoly"` (`regularPolygonBoard(n)`, a regular n-gon), `"dodeca"` (`dodecahedronBoard()`), and `"icosa"` (`icosahedronBoard()`) all have inherently-irrational unit-edge-length coordinates, which `BoardConfig::embed` (`unsigned` integers only) can't represent exactly like every other board type here can - so these three use `emb_dim = 0` with an empty `embed[]` per node instead: adjacency alone is exact and complete for gameplay, and no featurizer needs real coordinates for them (CNN/UNet require `emb_dim == 2` and throw otherwise - see `cnn.cpp`/`unet.cpp` - and neither is grid-shaped anyway, so nothing is lost by omitting positions).
