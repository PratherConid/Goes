...
- `POST /move` — runs MCTS from the given position and returns the chosen move.
  - **Request fields:**
    - `config`: object with the game configuration (matches `shared/types.ts`'s `GameConfig.toJSON()` wire shape):
      - `boardType`: `"rect"` | `"rectd"` | `"cublat"` | `"hcub"` | `"tri"` | `"regpoly"` | `"tetra"` | `"octa"` | `"dodeca"` | `"icosa"` | `"trihex"` | `"hex"` | `"hexdel"` | `"snubsq"` | `"snubsqtri"` | `"twsq"` | `"gtsq"` | `"star"`
        - `"regpoly"` (`regularPolygonBoard(n)`, a regular n-gon), `"tetra"` (`tetrahedronBoard()`, a regular tetrahedron - 4 vertices, all mutually adjacent), `"octa"` (`octahedronBoard()`, a regular octahedron - 6 vertices, 8 triangular faces), `"dodeca"` (`dodecahedronBoard()`), `"icosa"` (`icosahedronBoard()`), and `"star"` (`starBoard(n)`, 1 center node connected to n outer nodes) all have inherently-irrational unit-edge-length coordinates, which `BoardConfig::embed` (`unsigned` integers only) can't represent exactly like every other board type here can - so all six use `emb_dim = 0` with an empty `embed[]` per node instead: adjacency alone is exact and complete for gameplay, and no featurizer needs real coordinates for them (CNN/UNet require `emb_dim == 2` and throw otherwise - see `cnn.cpp`/`unet.cpp` - and none is grid-shaped anyway, so nothing is lost by omitting positions). A side-length-w subdivision of `tetra`'s 4 triangular faces (or `octa`'s 8) is available via the `triangleForm`/`triform` board modifier (`{kind: "TriangleForm", w: int}`) instead of being built into either directly. The `globalCentralize`/`gcent` board modifier (`{kind: "GlobalCentralize"}`) adds a hub node connected to every existing node, at the board's barycenter - it likewise always forces `emb_dim = 0` (a global hub isn't grid-shaped either), regardless of the board it's applied to. The `sqOctarize`/`sqocta` board modifier (`{kind: "SqOctarize"}`) replaces every square (4-cycle with no diagonal edges) with an octahedron - two new apex nodes per square, each connected to that square's 4 corners - and also always forces `emb_dim = 0`, since the TS side's apex positions are offset along a genuinely new dimension by an inherently irrational (Euclidean) distance. The `scaleBoard`/`scale` board modifier (`{kind: "Scale", factor: number}`) multiplies every node's position by `factor` on the TS side; here it's a no-op (returns the board unchanged) - `embed` coordinates must stay exact integers and C++ never renders them anyway, so scaling would only lose precision for no benefit.
      - `boardArgs`: one entry per positional dimension arg the board type expects, each `{kind: "Number", value: int}` or `{kind: "CommaSeparatedNumbers" | "ZeroOneList", values: int[]}` (e.g. `[{kind: "Number", value: 9}, {kind: "Number", value: 9}]` for a 9×9 rect board) — see `shared/boardConfig.ts`'s `BoardArgEntry`
...
| `--num-attn-layers N` | `8` | Transformer cross-attention layer count (separate flag from `--num-layers` - not shared with GNN) |
...
| `--num-selfplay-models N` | `1` | Target size of the active self-play model set - see **Tournament-Based Model Selection**, below |
| `--initialization-iters N` | `0` | First N iterations (raw iter count) force a singleton `active_models` of `model_var`'s latest snapshot - see **Tournament-Based Model Selection**, below |
| `--num-tournament-models N` | `6` | Participants per tournament: the active models plus randomly sampled challengers, filling up to this many total - must be `> --num-selfplay-models` |
| `--undefeated-threshold N` | `3` | Consecutive all-prevail tournaments before retiring an active model into the undefeated list - see **Tournament-Based Model Selection**, below |
...
2. **History set**: every *past* state's `(D,)` embedding forms an unordered set, used as-is (no self-attention refinement pass). No positional/recency encoding is ever added, so the set stays permutation-equivariant (order-symmetric) by construction. A learned "history sentinel" token is prepended (never masked) so attention always has at least one valid key, even for a genesis state with zero past plies.
3. **Cross-attention**: the *current* state's `(D,)` embedding is the query (it never joins the history set itself), attending directly over the (unrefined) history set as keys/values, stacked `--num-attn-layers` times - only the query evolves layer-to-layer, the key/value set stays fixed.
...
There is a single continuously-trained model, `model_var` (`train.cpp`), trained every iteration
with one persistent optimizer (`model_optimizer`) spanning the whole run - no per-iteration resets,
so Adam's momentum/variance state carries across every training step `model_var` ever takes.
`active_models` (`ModelSnapshots`, `train.cpp`) and `ChallengerPool` never hold `model_var` itself -
only frozen, independent snapshots of its weights at various points in time, each tagged with the
training iteration that captured it. Every player in every self-play game is evaluated by one of the
currently-**active** snapshots; every time a self-play game is created (both the initial game pool
and each slot's replenishment once a game ends), each player is **independently** randomly assigned
one of the active snapshots, so a single game can have different players evaluated by different
snapshots.

**Per-iteration training**: each iteration, after self-play, `model_var` is trained for one
iteration's worth of backprop directly (`run_training_iteration()`), continuing the same
gradient-descent trajectory it's always been on. A frozen, independent clone of its resulting
weights (`clone_model()`) is then appended to the challenger pool as a fresh entry - the pool never
trains anything itself, it just accumulates periodic captures of `model_var`'s progress for
...
**`--initialization-iters N`** overrides the above for the first N iterations, by raw iteration
count: instead of growing, `active_models` is forced to a fresh singleton of `model_var`'s current
snapshot every iteration. This exists to avoid a pathological failure mode (see **Notes**, below):
one of the first few models can end up locked in as the long-term active model, and later models -
trained on self-play data generated by these same models - can end up structurally unable to ever
beat it in a tournament. Since it's a raw `iter` comparison, a `--resume`d run (or one continuing
past `--retrain`'s replay phase) whose `iter` is already `>= N` never enters this window at all - no
special-casing needed. Tournaments never fire during the window either, since the challenger pool
never receives anything to draw wildcards from. Off (`0`) by default.
...
the tournament will run: `assign_by_sequence()` concatenates enough independent random shuffles of
the participant list (each containing every participant exactly once) to cover every slot in the
whole tournament, then hands out consecutive chunks of that single sequence, one game at a time -
...
marked `[promoted]` (genuinely new - wasn't in `active_models` immediately before this tournament)
or `[prevailed]` (was already active and simply won again) - either way, those become the new
`active_models`, wholesale-replacing the old set. Only `[promoted]` winners get a `.pt` file written
...
**`--undefeated-threshold N`** guards against `active_models` entrenching itself indefinitely purely
by winning (see **Notes**, below, for the failure mode this complements `--initialization-iters`
against) - a counter tracks consecutive tournaments where `active_models`' composition didn't change
at all (every active model simply prevailed). Once it reaches N, one active model is picked at
random, retired onto a separate `undefeated` stack, and replaced by the tournament's next-best
challenger (which gets its own `.pt` file, same as any newly-promoted model), and the counter resets.
Retired models aren't wasted: every tournament, in addition to the usual `--num-tournament-models`
participants, the tail of the `undefeated` stack also plays as a bonus entrant, judged against the
same raw top-`--num-selfplay-models` cutoff as everyone else - if it's still in that raw winner list
it goes back onto the stack (marked `[undefeated]` in the standings) to keep proving itself
indefinitely; otherwise it's marked `[defeated]` and dropped for good. Either way, the bonus entrant
never itself becomes a real active model - real promotion is always computed with it excluded
entirely, so its own presence can only ever bump a *challenger* up to fill the resulting gap, never
displace a genuinely active model. Off in practice for short runs at the default of `3`.
...
- Self-attention over the Transformer's history set (query=key=value over the full `(T, D)` sequence) produces an `O(T²)` attention-weight tensor per layer, so peak GPU memory grew *quadratically* with history length - on `forcedPassOnly` boards with no move bound, a single unusually long game entering self-play or the replay buffer could blow up memory (confirmed empirically: fitting `peak_MB ≈ 338 + 0.0216·Tmax²` matched measured peaks within ~9% across two codebase states and a 3x range of iteration counts). This is why the current architecture (see **Transformer Architecture**, above) uses cross-attention only, whose query has length 1 and so costs `O(T)` per layer - removing the self-attention stack measured a ~3.6-4x reduction in peak memory at matched history length and batch size, and replaced the quadratic growth with linear.
- When `--num-selfplay-models` is greater than 1, we're observing that none of the newly trained challengers can outplay the existing active models in tournaments - the active set never turns over. Speculated reason: with multiple active models, each self-play game randomly assigns different snapshots to different player slots (see `assign_random_models()`), which lowers the quality/consistency of the generated games each individual model is trained on relative to every player being the same, single active model - degrading the training signal enough that new challengers can't catch up. Not yet root-caused; `--num-selfplay-models 1` sidesteps it, hence the default change.
- A real 9x9 CNN run (`D:/tune_1.log`) showed the *initial* (randomly-initialized) model winning every one of 7 consecutive tournaments across 111+ iterations - `active_models` never turned over even with `--num-selfplay-models 1`. This looks like a pathological entrenchment failure mode rather than the first model actually being strong: since self-play data is always generated using whatever's currently active, every later model is trained on data shaped by that same active model, which can make it structurally hard for any of them to ever beat it head-to-head in a tournament, regardless of `model_var`'s own underlying progress. `--initialization-iters` (see **Tournament-Based Model Selection**, above) works around this by refreshing `active_models` to `model_var`'s latest snapshot every iteration for a warm-up period, rather than letting one of the first few models lock in as the long-term active baseline. `--initialization-iters` only guards the start of a run, though - the same entrenchment could in principle happen later too, with a model that legitimately keeps winning long enough to stall out any newer line's development. `--undefeated-threshold` (see **Tournament-Based Model Selection**, above) is the defensive complement: it forces periodic turnover regardless of *why* `active_models` has stopped changing.
