  ...
  - **Request fields:**
    - `config`: object with the game configuration (matches `shared/types.ts`'s `GameConfig.toJSON()` wire shape):
      - `boardType`: `"rect"` | `"rectd"` | `"cublat"` | `"hcub"` | `"tri"` | `"regpoly"` | `"tetra"` | `"dodeca"` | `"icosa"` | `"trihex"` | `"hex"` | `"hexdel"` | `"snubsq"` | `"snubsqtri"` | `"twsq"` | `"gtsq"`
        - `"regpoly"` (`regularPolygonBoard(n)`, a regular n-gon), `"tetra"` (`tetrahedronBoard(w)`, 4 triangular-board(w) faces glued together), `"dodeca"` (`dodecahedronBoard()`), and `"icosa"` (`icosahedronBoard()`) all have inherently-irrational unit-edge-length coordinates, which `BoardConfig::embed` (`unsigned` integers only) can't represent exactly like every other board type here can - so all four use `emb_dim = 0` with an empty `embed[]` per node instead: adjacency alone is exact and complete for gameplay, and no featurizer needs real coordinates for them (CNN/UNet require `emb_dim == 2` and throw otherwise - see `cnn.cpp`/`unet.cpp` - and none is grid-shaped anyway, so nothing is lost by omitting positions).
