...
struct Args {
    ...
    // For the first N iterations (raw iter count - see main()'s Training section), active_models is
    // forced to a singleton of model_var's latest snapshot every iteration instead of following the
    // usual growth-phase/tournament-promotion rules - avoids letting one of the first few models get
    // locked in as the long-term active model, which later models (trained on self-play data
    // generated by these models) can end up pathologically unable to ever beat in a tournament. A
    // --resume'd run (or one past --retrain's replay phase) whose iter is already >= N simply never
    // enters this window. 0 (default) disables it entirely.
    ...
    // After this many consecutive tournaments where every active model simply prevails (no
    // promotion at all), one active model is forcibly retired into the "undefeated" list and
    // replaced by the next-best challenger - guards against a model entrenching itself indefinitely
    // just by winning fair and square. See main()'s tournament block and ai/Readme.md's
    // "Tournament-Based Model Selection".
    int undefeated_threshold = 3;
...
static void print_usage(const char* prog) {
    std::cout << "Usage: " << prog << " [options]\n"
              ...
              << "  --initialization-iters N  First N iterations (raw iter count) force a singleton\n"
              << "                            active_models of model_var's latest snapshot (default: 0)\n"
              << "  --num-tournament-models N Participants per tournament (active models + random\n"
              << "                            challengers) - must be > --num-selfplay-models\n"
              << "                            (default: 6)\n"
              << "  --undefeated-threshold N  Consecutive all-prevail tournaments before retiring an\n"
              << "                            active model into the undefeated list (default: 3)\n"
...
static Args parse_args(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (a == "--initialization-iters") {
            args.initialization_iters = std::stoi(argv[++i]);
            if (args.initialization_iters < 0) {
                std::cerr << "--initialization-iters must be >= 0 (got " << args.initialization_iters << ")\n";
                std::exit(1);
            }
        }
        ...
        else if (a == "--undefeated-threshold") {
            args.undefeated_threshold = std::stoi(argv[++i]);
            if (args.undefeated_threshold < 1) {
                std::cerr << "--undefeated-threshold must be >= 1 (got " << args.undefeated_threshold << ")\n";
                std::exit(1);
            }
        }
...
static std::string effective_arch(const Args& args, const std::string& board_kind) {
    bool grid2d_supported = (board_kind == "rect" || board_kind == "rectd" || board_kind == "tri" ||
                          ...
                          board_kind == "snub" || board_kind == "twsq" || board_kind == "gtsq");
    if (args.net_arch == "cnn") {
        if (!grid2d_supported) {
            std::cerr << "Error: --net-arch cnn is not supported for board type '" << board_kind
                      << "'. CNN requires a 2D grid embedding (rect/rectd/tri/trihex/hex/hexdel/snub/twsq/gtsq).\n";
            std::exit(1);
        }
        return "cnn";
    }
    if (args.net_arch == "unet") {
        if (!grid2d_supported) {
            std::cerr << "Error: --net-arch unet is not supported for board type '" << board_kind
                      << "'. UNet requires a 2D grid embedding (rect/rectd/tri/trihex/hex/hexdel/snub/twsq/gtsq).\n";
            std::exit(1);
        }
        return "unet";
    }
...
int main(int argc, char* argv[]) {
    ...
    // Consecutive tournaments where active_models' composition didn't change at all (every active
    // model prevailed) - drives --undefeated-threshold's forced retirement, below.
    int consecutive_all_prevails = 0;
    // Stack (back() = tail) of models retired by --undefeated-threshold for winning too many
    // tournaments in a row - each tournament re-tests the tail as a bonus participant (see main()'s
    // tournament block), so a model that's still genuinely excellent can keep proving it indefinitely
    // without permanently occupying an active slot.
    ModelSnapshots undefeated;
    ...
    for (; iter < args.iterations; iter++) {
        ...
        if (iter < args.initialization_iters) {
            // Warm-up: force active_models to {model_var's latest snapshot}, refreshed every
            // iteration, instead of letting one of the first few models get locked in as the
            // long-term active model - later models, trained on self-play data generated by these
            // models, can end up pathologically unable to ever beat it in a tournament (see
            // ai/Readme.md's Notes). A --resume'd run whose iter is already past this threshold just
        ...
        if ((iter + 1) % args.tournament_every == 0) {
            ...
            if ((int)challengers.entries().size() < wildcards_needed) {
                ...
                // Bonus participant: the tail of the undefeated stack (a model previously retired by
                // --undefeated-threshold), on top of the usual num_tournament_models - gets a chance
                // to keep proving itself instead of sitting out forever. Popped now, conditionally
                // pushed back below once we know whether it survived.
                bool has_undefeated = !undefeated.empty();
                std::optional<std::pair<int, AnyModel>> undefeated_entry;
                if (has_undefeated) {
                    undefeated_entry = std::move(undefeated.back());
                    undefeated.pop_back();
                    participants.push_back(*undefeated_entry);
                }
                int undefeated_top = has_undefeated ? undefeated_entry->first : std::numeric_limits<int>::min();
                ...
                std::vector<int> winner_ids;  // "raw" winners, across all participants incl. undefeated
                ...
                // The undefeated bonus participant's own fate is judged against the raw winner list -
                // survives (goes back on the stack) or is gone for good, independent of real
                // promotion below, which excludes it entirely either way (see real_winner_ids).
                bool undefeated_survived = false;
                if (has_undefeated) {
                    undefeated_survived = std::find(winner_ids.begin(), winner_ids.end(), undefeated_top) != winner_ids.end();
                    if (undefeated_survived) undefeated.push_back(std::move(*undefeated_entry));
                }

                // Real promotion excludes the undefeated bonus participant entirely (win or lose) -
                // this is what "gives the slot to the next runner-up" for a surviving bonus
                // participant, with no separate backfill step needed.
                std::vector<int> adjusted_order;
                for (auto& [id, avg] : ranked) if (id != undefeated_top) adjusted_order.push_back(id);
                for (int id : unranked)        if (id != undefeated_top) adjusted_order.push_back(id);
                int top_k_real = std::min(args.num_selfplay_models, (int)adjusted_order.size());
                std::vector<int> real_winner_ids(adjusted_order.begin(), adjusted_order.begin() + top_k_real);

                auto is_winner = [&](int id) {
                    return std::find(real_winner_ids.begin(), real_winner_ids.end(), id) != real_winner_ids.end();
                };
                ...
                auto winner_tag = [&](int id) {
                    if (has_undefeated && id == undefeated_top)
                        return undefeated_survived ? "  [undefeated]" : "  [defeated]";
                ...
                // Promote: real_winner_ids become the new active_models (never the undefeated bonus
                // participant, win or lose - see above). Only save the ones genuinely new (is_new) -
                // an incumbent that simply prevailed again was already saved the tournament it was
                // first promoted, so re-saving it here would just duplicate that file.
                ModelSnapshots new_active;
                new_active.reserve(real_winner_ids.size());
                ...
                // --undefeated-threshold: if active_models' composition didn't change at all this
                // tournament (every real winner is an old active model), count it - after enough
                // consecutive such tournaments, retire one active model at random into the undefeated
                // stack and promote the next-best challenger in its place, so a model can't entrench
                // itself indefinitely purely by winning (see ai/Readme.md's Notes).
                bool all_prevailed = [&] {
                    if (real_winner_ids.size() != old_active_ids.size()) return false;
                    auto a = real_winner_ids, b = old_active_ids;
                    std::sort(a.begin(), a.end()); std::sort(b.begin(), b.end());
                    return a == b;
                }();
                consecutive_all_prevails = all_prevailed ? consecutive_all_prevails + 1 : 0;

                if (all_prevailed && consecutive_all_prevails >= args.undefeated_threshold
                    && (int)adjusted_order.size() > top_k_real) {
                    int runner_up_id = adjusted_order[top_k_real];
                    auto runner_up_it = std::find_if(participants.begin(), participants.end(),
                                                       [&](auto& e) { return e.first == runner_up_id; });
                    std::uniform_int_distribution<size_t> dist(0, new_active.size() - 1);
                    size_t retire_idx = dist(rng);
                    if (args.verbosity >= 1)
