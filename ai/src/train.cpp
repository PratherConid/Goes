    ...
    if (args.net_arch == "cnn") {
        if (!grid2d_supported) {
            std::cerr << "Error: --net-arch cnn is not supported for board type '" << board_kind
                      << "'. CNN requires a 2D grid embedding (rect/rectd/tri/trihex/hex/hexdel/twsq/gtsq).\n";
            std::exit(1);
        }
        return "cnn";
    }
    if (args.net_arch == "unet") {
        if (!grid2d_supported) {
            std::cerr << "Error: --net-arch unet is not supported for board type '" << board_kind
                      << "'. UNet requires a 2D grid embedding (rect/rectd/tri/trihex/hex/hexdel/twsq/gtsq).\n";
            std::exit(1);
        }
        return "unet";
    }
