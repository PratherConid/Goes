// Regression tests for sierpinskiSimplex: the Sierpinski dim-simplex gasket board type. n=0 is a
// single node; n=1 is a unit-edge regular dim-simplex; n>1 is dim+1 copies of n-1 glued at touching
// corners (the classic "hole" - the sub-simplex a full subdivision would keep at the non-corner
// positions - is never built). dim=1 has no hole to remove at all, so it degenerates to a plain
// bisected line; dim=2 is the classic Sierpinski triangle.
...
test('n=0 is a single node at the origin, with no edges, regardless of dim', () => {
    for (const dim of [1, 2, 3, 4]) {
        const bc = sierpinskiSimplex(dim, 0);
        assert.equal(bc.N, 1, `dim=${dim}`);
        assert.deepEqual(bc.emb.pos, [new Array(dim).fill(0)], `dim=${dim}`);
        assert.equal(bc.adj[0][0], 0, `dim=${dim}`);
    }
});
...
test('rejects a non-positive/non-integer dim, or a negative/non-integer n', () => {
