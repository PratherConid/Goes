...
// strB(...) (see the coin/str_* presets below) needs its fonts loaded via shared/glyphRaster.ts's
// own registry before it can evaluate - normally done once at app startup (src/main.ts,
// server/src/index.ts), so it's done here too, synchronously, before any preset gets evaluated.
for (const name of BUNDLED_FONT_NAMES) {
    const buf = fs.readFileSync(path.join(__dirname, '..', 'public', 'fonts', `${name}.ttf`));
    loadFont(name, buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength));
}
...
const BOARD_PRESET_GOLDEN: Record<string, { N: number; edges: number }> = {
    ...
    'coin_5_4_9.5.cleg': { N: 808, edges: 1616 },
    ...
    'str_go_40_40.cleg': { N: 839, edges: 1501 },
