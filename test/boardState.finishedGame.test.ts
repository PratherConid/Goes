...
import { makeBoardState, makeGameConfig, soloStoneTurns } from './boardStateHelpers.ts';
...
// The GameConfig counterpart of playShortGame()'s own BoardState.
const shortGameConfig = () => makeGameConfig('rectB(1, 1);', soloStoneTurns(2));

test('BoardState.fromFinishedGame reconstructs the same final view as the live game', () => {
    const { bc, bs } = playShortGame();
    const fg = new FinishedGame(
        shortGameConfig(), bs.moveInfos().map(m => ({ pos: m.pos, stone: m.stone })), new Map(bs.resigns));
