...
export async function aiMove(url: string, body: unknown): Promise<unknown> {
    ...
    if (!resp.ok) {
        ...
        // {"error": "<specific message>"} on failure - e.g. "No checkpoint
        // found matching the requested config" when the model is missing.
        // Propagate that instead of a generic message so callers can tell a
        // missing-model failure apart from other /move-time errors.
        let message = 'AI engine error';
        try {
