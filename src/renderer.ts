...
const _boardConfigNames = [
    ...
    'threeRing_5_10.5_7_12.5_9_14.5', 'biborder_3_tri_3_fractaldrop_3_0.05', 'coin_5_4_9.5',
    'str_go_40_40',
    ...
    private _initCommandsPanel() {
        ...
        this.commandReferenceBoardTypesPanel.innerHTML = table(
            Object.values(PrescribedBoardMap)
                ...
                .concat([
                    row('subHcublatB(bounds, cond)/subHcublatB(bounds, cond, econd)/subHcublatB(bounds, cond, adjFn)',
                        ...
                        + 'coordinate array - decides which of those points actually become nodes (same for '
                        + 'all three overloads). Edges differ per overload: the 2-arg one keeps every plain '
                        + 'grid edge (connecting points whose coordinates differ by exactly 1 in one '
                        + 'dimension); the econd overload additionally gates each such edge through '
                        + 'econd(a, b) (the two candidate endpoints\' own coordinate arrays); the adjFn '
                        + 'overload discards the grid edges entirely and instead calls adjFn(point) on every '
                        + 'surviving node, connecting it to whichever OTHER points (named by their own '
                        + 'absolute coordinates) it returns - a returned point matching no surviving node is '
                        + 'silently ignored. Every overload re-centers around the surviving nodes\' own '
                        + 'bounding box'),
                    row('strB(str, fontName, hScale, vScale)',
                        'Renders str in fontName (one of "Roboto"/"OpenSans"/"Montserrat"/"Lato", '
                        + 'each "-Regular" or "-Bold" - see shared/glyphRaster.ts\'s own '
                        + 'BUNDLED_FONT_NAMES, loaded from public/fonts/ at app startup) at '
                        + 'hScale/vScale cells-per-em (independent horizontal/vertical resolution - '
                        + 'every character shares the same row count, but each gets its own column '
                        + 'count from its own advance width, so a narrow "i" and a wide "m" come out '
                        + 'proportionally sized when placed side by side), then returns the '
                        + 'node-induced subgraph of a plain rectangular grid kept wherever the '
                        + 'rendered bitmap is on'),
                ])
                .join('\n            '),
        );
        ...
        this.commandReferenceBuiltinFunctionsPanel.innerHTML = table(`
            ...
            ${row('trunc(x)', 'x rounded toward zero, dropping any fractional part')}
            ${row('round(x)', 'x rounded to the nearest integer (halves round toward +Infinity)')}
            ...
            ${row('rasterizeString(str, fontName, hScale, vScale)',
                'Renders str in fontName (one of "Roboto"/"OpenSans"/"Montserrat"/"Lato", each '
                + '"-Regular" or "-Bold" - see shared/glyphRaster.ts\'s own BUNDLED_FONT_NAMES) at '
                + 'hScale/vScale cells-per-em, returning the rendered bool[][] bitmap directly '
                + '(bm[row][col], row 0 at the top) rather than building a board from it - the same '
                + 'rendering strB(...) builds a board from, for a cleg program that wants to inspect '
                + 'or combine the bitmap itself first')}
