...
/**
 * Renders `str` in `fontName` (one of shared/glyphRaster.ts's own BUNDLED_FONT_NAMES, already
 * loaded via loadFont() at app startup - see that file's own "font registry" section) at
 * `hScale`/`vScale` cells-per-em via shared/glyphRaster.ts's own rasterizeString (which does the
 * actual rendering and whose own doc comment, and rasterizeGlyph's, explain these two independent
 * scales and the row/column conventions), then returns the node-induced subgraph of a plain
 * rectangular grid (hypercuboidBoard(2, ...), the same full 4-neighbor lattice rectangularBoard
 * itself builds on) kept exactly where the rendered bitmap is on.
 */
export function stringBoard(str: string, fontName: string, hScale: number, vScale: number): BoardConfig {
    assert(hScale > 0 && vScale > 0, `hScale and vScale must be positive, got hScale=${hScale} vScale=${vScale}`);

    const bitmap = rasterizeString(getFont(fontName), str, hScale, vScale);
    const rows = bitmap.length;
    const totalCols = bitmap[0].length;

    const bc = hypercuboidBoard(2, [totalCols, rows]);
    const kept = new Set<number>();
    for (let row = 0; row < rows; row++)
        for (let col = 0; col < totalCols; col++) {
            if (!bitmap[row][col]) continue;
            // bitmap row 0 is the TOP (see rasterizeString/rasterizeGlyph's own doc comments), but
            // hypercuboidBoard's own y coordinate increases UPWARD on screen (see
            // computeStarPoints's own doc comment on the originY - y*cell screen transform) - so
            // row 0 maps to the LARGEST y index here, not the smallest.
            const y = rows - 1 - row;
            kept.add(col + y * totalCols); // hypercuboidBoard(2, ...)'s own fullIdx == board index (no meshdim culling)
        }
    return nodeInducedSubgraph(bc, kept);
}
