...
import {
    ...
    make, nodeInducedSubgraph, edgeInducedSubgraph, nodeEdgeInducedSubgraph, stringBoard,
} from './boardConfig.js';
import {
...
import { rasterizeString, getFont } from './glyphRaster.js';
...
// `strB(str, fontName, hScale, vScale)`: mirrors shared/boardConfig.ts's stringBoard() - a genuine
// board constructor, but hand-registered here (rather than folded into the PrescribedBoardMap loop
// above) since its own `str`/`fontName` arguments are arbitrary strings, a shape BoardArgType has no
// case for (Number/CommaSeparatedNumbers/ZeroOneList only).
BUILTIN_FUNCTIONS['strB'] = {
    checkCall: fixedSignature([STRING_TYPE, STRING_TYPE, NUMBER_TYPE, NUMBER_TYPE], EGR_TYPE),
    call: ([str, fontName, hScale, vScale]) => ({
        kind: 'egr',
        value: stringBoard(
            (str as { value: string }).value,
            (fontName as { value: string }).value,
            (hScale as { value: number }).value,
            (vScale as { value: number }).value,
        ),
    }),
};

const BOOL_TYPE: ClegType = { kind: 'bool' };
const BOOL_ROW_TYPE: ClegType = { kind: 'array', elem: BOOL_TYPE };
const BOOL_GRID_TYPE: ClegType = { kind: 'array', elem: BOOL_ROW_TYPE };

// `rasterizeString(str, fontName, hScale, vScale)`: mirrors shared/glyphRaster.ts's
// rasterizeString() - the same rendering strB/stringBoard build a board from, but returned directly
// as a bool[][] bitmap (`grid[row][col]`, row 0 at the top - see rasterizeString's own doc comment)
// rather than turned into a board, so a cleg program can inspect/combine it itself before deciding
// what (if anything) to build from it.
BUILTIN_FUNCTIONS['rasterizeString'] = {
    checkCall: fixedSignature([STRING_TYPE, STRING_TYPE, NUMBER_TYPE, NUMBER_TYPE], BOOL_GRID_TYPE),
    call: ([str, fontName, hScale, vScale]) => {
        const bitmap = rasterizeString(
            getFont((fontName as { value: string }).value),
            (str as { value: string }).value,
            (hScale as { value: number }).value,
            (vScale as { value: number }).value,
        );
        return {
            kind: 'array', elem: BOOL_ROW_TYPE,
            value: bitmap.map((row): ClegValue => ({
                kind: 'array', elem: BOOL_TYPE,
                value: row.map((b): ClegValue => ({ kind: 'bool', value: b })),
            })),
        };
    },
};
...
// `trunc(x)`/`round(x)`: fixed-signature `number -> number` like abs above - Math.trunc/Math.round
// are both already total (defined and finite for every finite `x`), so neither needs a validity
// check the way sqrt's own negative-input case does.
BUILTIN_FUNCTIONS['trunc'] = {
    checkCall: fixedSignature([NUMBER_TYPE], NUMBER_TYPE),
    call: ([x]) => ({ kind: 'number', value: Math.trunc((x as { value: number }).value) }),
};
BUILTIN_FUNCTIONS['round'] = {
    checkCall: fixedSignature([NUMBER_TYPE], NUMBER_TYPE),
    call: ([x]) => ({ kind: 'number', value: Math.round((x as { value: number }).value) }),
};
...
const HCUBLAT_BOUNDS_TYPE: ClegType = { kind: 'array', elem: { kind: 'array', elem: NUMBER_TYPE } };
const HCUBLAT_POINT_TYPE: ClegType = { kind: 'array', elem: NUMBER_TYPE };
...
function buildSubHcublat(
    ...
    for (let bi = 0; bi < N; bi++) {
        ...
        for (let i = 0; i < k; i++) {
            ...
            if (!econd(pos[bi], pos[nbi])) continue;
            adj[bi][nbi] = 1;
            adj[nbi][bi] = 1;
        }
    }
...
function econdCallback(econdVal: ClegValue, funcs: UserFuncTable): (a: number[], b: number[]) => boolean {
    ...
    const fn = funcs[econd.name];
    return (a, b) => {
        const aArg: ClegValue = { kind: 'array', elem: NUMBER_TYPE, value: a.map(v => ({ kind: 'number', value: v })) };
        const bArg: ClegValue = { kind: 'array', elem: NUMBER_TYPE, value: b.map(v => ({ kind: 'number', value: v })) };
        return (callUserFunction(fn, fillHoles(econd.boundArgs, [aArg, bArg]), funcs) as { value: boolean }).value;
    };
}
...
function subHcublatCheckCall(callee: string, argTypes: ClegType[]): ClegType {
    ...
    if (!typeEquals(argTypes[0], HCUBLAT_BOUNDS_TYPE))
